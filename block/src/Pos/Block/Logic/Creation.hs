...
createMainBlockAndApply pm sId pske =
  ...
  where
    createAndApply tip =
        DB.getTipHeader >>= createMainBlockInternal pm sId pske >>= \case
...
createMainBlockInternal ::
    ...
    -> BlockHeader
    -> m (Either Text MainBlock)
createMainBlockInternal pm sId pske tipHeader = do
...
applyCreatedBlock pm pske createdBlock = applyCreatedBlockDo False createdBlock
  where
    ...
    fallback reason = do
        ...
        DB.getTipHeader >>= createMainBlockInternal pm slotId pske >>= \case
            Left err ->
                assertionFailed $
