...
import           Pos.DB.Class (MonadDB (..), MonadDBRead (..), Serialized (..), SerializedBlock,
                               SerializedUndo, SerializedBlund, getBlock, getBlockWithExtRep, getDeserialized)
...
getBlundWithExtRep :: MonadDBRead m => HeaderHash -> m (Maybe (Block 'AttrExtRep, Undo))
getBlundWithExtRep x =
    runMaybeT $
    (,) <$> MaybeT (getBlockWithExtRep x)
        <*> MaybeT (getUndo x)
